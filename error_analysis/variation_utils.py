...
def plot_parameter_sweep(
    ...
    """
    Plot median Frobenius norm vs. parameter value for one parameter sweep.

    Parameters
    ----------
    csv_path : str | Path
        Path to the CSV generated by sweep_results().
    parameter_name : str
        Parameter to plot (e.g. "K", "n", "r", "p", "snr").
    matrices : list[str] | None
        Matrix names to include. If None, plots all matrices.
    ...
    # Filter parameter
    df = df[df["parameter_name"] == parameter_name]

    # Filter matrices if requested
    if matrices is not None:
        df = df[df["matrix_name"].isin(matrices)]

    # Compute summary statistics across seeds
    summary = (
        df.groupby(["matrix_name", "parameter_value"])["frob_norm"]
        .agg(
            median="median",
    ...
    for matrix_name, group in summary.groupby("matrix_name"):

        group = group.sort_values("parameter_value")

        yerr = [
            group["median"] - group["q25"],
            group["q75"] - group["median"],
        ]
    ...
    ax.set_title(f"{parameter_name} Sweep")
    ax.set_xlabel(parameter_name)
    ax.set_ylabel("Median Frobenius Norm")
    ax.grid(True)
    ax.legend()
